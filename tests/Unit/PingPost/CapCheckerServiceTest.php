...
function makeAcceptedPost(Integration $integration, float $price = 10.0, ?Carbon\Carbon $at = null): PostResult
{
  ...
  if ($at) {
    \Illuminate\Support\Facades\DB::table('post_results')
      ->where('id', $result->id)
...
it('returns true when revenue meets the daily cap', function () {
  $integration = Integration::factory()->pingPost()->create();
  BuyerCapRule::create(['integration_id' => $integration->id, 'period' => 'day', 'max_revenue' => 20.0]);
  $integration->load('capRules');

  makeAcceptedPost($integration, 10.0);
  makeAcceptedPost($integration, 10.0);
...
it('returns false when revenue is below the daily cap', function () {
  $integration = Integration::factory()->pingPost()->create();
  BuyerCapRule::create(['integration_id' => $integration->id, 'period' => 'day', 'max_revenue' => 50.0]);
  $integration->load('capRules');
...
it('returns true if any cap rule is exceeded (OR semantics)', function () {
  $integration = Integration::factory()->pingPost()->create();
  BuyerCapRule::create(['integration_id' => $integration->id, 'period' => 'day', 'max_leads' => 100, 'max_revenue' => 5.0]);
  $integration->load('capRules');

  makeAcceptedPost($integration, 5.0); // revenue hits cap, leads is only 1
