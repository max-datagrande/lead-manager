...
it('returns fixed_price regardless of bid for FIXED type', function () use ($config) {
  $c = $config(['price_source' => PriceSource::FIXED, 'fixed_price' => 15.0, 'min_bid' => null]);

  $resolver = app(PriceResolverService::class);

  expect($resolver->resolvePrice($c, 0.0))->toBe(15.0)->and($resolver->resolvePrice($c, 99.99))->toBe(15.0);
});

// ─── Min bid ─────────────────────────────────────────────────────────────────

it('returns bid when bid meets min_bid threshold', function () use ($config) {
  $c = $config(['price_source' => PriceSource::RESPONSE_BID, 'min_bid' => 5.0, 'fixed_price' => null]);

  expect(app(PriceResolverService::class)->resolvePrice($c, 5.0))->toBe(5.0);
...
it('returns null when bid is below min_bid', function () use ($config) {
  $c = $config(['price_source' => PriceSource::RESPONSE_BID, 'min_bid' => 10.0, 'fixed_price' => null]);
...
it('isPriceAcceptable returns true for FIXED type always', function () use ($config) {
  $c = $config(['price_source' => PriceSource::FIXED, 'min_bid' => null]);
...
it('isPriceAcceptable uses min_bid threshold for MIN_BID type', function () use ($config) {
  $c = $config(['price_source' => PriceSource::RESPONSE_BID, 'min_bid' => 5.0]);

  $resolver = app(PriceResolverService::class);
  expect($resolver->isPriceAcceptable($c, 5.0))->toBeTrue();
...
it('resolves conditional price from first matching rule', function () use ($config) {
  $c = $config([
    'price_source' => PriceSource::CONDITIONAL,
...
it('returns null when no conditional rule matches', function () use ($config) {
  $c = $config([
    'price_source' => PriceSource::CONDITIONAL,
    'conditional_pricing_rules' => [['conditions' => [['field' => 'state', 'op' => 'eq', 'value' => 'CA']], 'price' => 20.0]],
  ]);
...
it('resolves conditional price with multiple conditions (AND)', function () use ($config) {
  $c = $config([
    ...
    'conditional_pricing_rules' => [
      [
        'conditions' => [['field' => 'state', 'op' => 'eq', 'value' => 'CA'], ['field' => 'age', 'op' => 'gte', 'value' => 25]],
        'price' => 30.0,
      ],
    ],
  ]);
...
it('returns null for POSTBACK pricing type (async resolution)', function () use ($config) {
  $c = $config(['price_source' => PriceSource::POSTBACK, 'fixed_price' => null, 'min_bid' => null]);
