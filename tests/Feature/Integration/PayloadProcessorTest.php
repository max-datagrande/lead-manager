...
it('replaces string tokens with lead values', function () {
  ...
  $result = app(PayloadProcessorService::class)->resolveTokens($env->request_body, $integration, $env, [
    'state' => 'TX',
    'cptype' => 'AMS',
    'client_ip' => '1.2.3.4',
    'user_agent' => '99',
  ...
  expect($payload['state'])->toBe('TX')->and($payload['consumerIP'])->toBe('1.2.3.4');
});
...
it('uses default_value when field is absent from lead data', function () {
  ...
  expect($payload['state'])->toBe('FL')->and($payload['pubcampaignid'])->toBe('14490');
});
...
it('strips quotes for integer data_type tokens in JSON', function () {
  ...
  expect($payload['score'])->toBe(42)->and($payload['score'])->toBeInt();
});
...
it('does not corrupt JSON when integer token appears inside a nested escaped string', function () {
  ...
  $result = app(PayloadProcessorService::class)->resolveTokens($nestedTemplate, $integration, $env, [
...
it('resolves legacy {field_name} tokens in headers and URLs', function () {
  ...
  $result = app(PayloadProcessorService::class)->resolveTokens($headersTemplate, $integration, $env, [
    'client_ip' => '9.9.9.9',
    'state' => 'TX',
    'user_agent' => '0',
    'cptype' => 'AMS',
  ...
  expect($headers['X-IP'])->toBe('9.9.9.9')->and($headers['X-State'])->toBe('TX');
});
