...
it('creates an integration with field_mappings', function () {
  ...
  expect($integration->tokenMappings)
    ->toHaveCount(1)
    ->and($integration->tokenMappings->first()->field_id)
    ->toBe($field->id)
    ->and($integration->tokenMappings->first()->data_type)
    ->toBe('string');
});
...
it('creates field_mapping with value_mapping', function () {
  ...
  expect($mapping->default_value)
    ->toBe('own')
    ->and($mapping->value_mapping)
    ->toBe(['own' => '1', 'rent' => '0']);
});
...
it('creates field_hashes per environment', function () {
  ...
  expect($prodEnv->fieldHashes)
    ->toHaveCount(1)
    ->and($prodEnv->fieldHashes->first()->is_hashed)
    ->toBeTrue()
    ->and($prodEnv->fieldHashes->first()->hash_algorithm)
    ->toBe('md5');
});
...
it('fails validation when field_id does not exist', function () {
  ...
  expect(
    fn() => $service->createIntegration(
      makeIntegrationPayload([
        'field_mappings' => [['field_id' => 99999, 'data_type' => 'string', 'default_value' => null]],
      ]),
    ),
...
it('syncs field_mappings on update — adds new, removes old', function () {
  ...
  expect($integration->tokenMappings)
    ...
    ->and($integration->tokenMappings->first()->data_type)
    ->toBe('integer')
    ->and($integration->tokenMappings->first()->default_value)
    ->toBe('0');
});
...
it('duplicates integration including field_mappings', function () {
  ...
  expect($copy->id)
    ...
    ->and($copy->is_active)
    ->toBeFalsy()
    ->and($copy->tokenMappings)
    ->toHaveCount(1)
    ->and($copy->tokenMappings->first()->field_id)
    ->toBe($field->id)
    ->and($copy->tokenMappings->first()->default_value)
    ->toBe('FL');
});
