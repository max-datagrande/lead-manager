...
it('returns 422 when neither fingerprint nor lead_id is provided', function () {
  ...
  postJson("/v1/share-leads/dispatch/{$workflow->id}", [], hostHeader())->assertStatus(422);
});

it('returns 404 when lead fingerprint does not exist', function () {
  $workflow = Workflow::factory()->create();

  postJson("/v1/share-leads/dispatch/{$workflow->id}", ['fingerprint' => 'does-not-exist'], hostHeader())->assertNotFound();
});

it('returns 404 when workflow does not exist', function () {
  postJson('/v1/share-leads/dispatch/999999', ['fingerprint' => 'x'], hostHeader())->assertNotFound();
});
