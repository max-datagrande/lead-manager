...
describe('Best Bid strategy', function () {
    it('posts to the highest bidder and marks dispatch as sold', function () {
        ...
        $buyer1 = Integration::factory()->pingPost()->withBuyerConfig()->create(['name' => 'Buyer A']);
        $buyer2 = Integration::factory()->pingPost()->withBuyerConfig()->create(['name' => 'Buyer B']);

        $workflow = Workflow::factory()->bestBid()->create();
        attachBuyers($workflow, [
            ['integration' => $buyer1, 'position' => 0],
            ['integration' => $buyer2, 'position' => 1],
        ]);

        Http::fake([
            'https://buyer.example.com/ping' => Http::sequence()
                ->push(fakePingAccepted(8.00))   // buyer1 bids 8
                ->push(fakePingAccepted(12.00)),  // buyer2 bids 12
    ...
    it('cascades to the next bidder when winner rejects post', function () {
        ...
        $workflow = Workflow::factory()->bestBid()->create(['cascade_on_post_rejection' => true, 'cascade_max_retries' => 2]);
        attachBuyers($workflow, [
            ['integration' => $buyer1, 'position' => 0],
            ['integration' => $buyer2, 'position' => 1],
        ]);
        ...
        Http::fake([
            'https://buyer.example.com/ping' => Http::sequence()
                ->push(fakePingAccepted(15.00))  // buyer1 wins bid
                ->push(fakePingAccepted(10.00)), // buyer2 second
    ...
    it('marks dispatch as not_sold when all buyers reject', function () {
        ...
        $workflow = Workflow::factory()->bestBid()->create(['cascade_max_retries' => 1]);
        attachBuyers($workflow, [['integration' => $buyer, 'position' => 0]]);

        Http::fake([
            'https://buyer.example.com/ping' => Http::response(fakePingAccepted(5.00)),
...
describe('Waterfall strategy', function () {
    it('posts to first buyer that accepts', function () {
        ...
        $buyer1 = Integration::factory()->postOnly()->withBuyerConfig(['pricing_type' => 'fixed', 'fixed_price' => 8.0])->create();
        $buyer2 = Integration::factory()->postOnly()->withBuyerConfig(['pricing_type' => 'fixed', 'fixed_price' => 6.0])->create();
    ...
    it('stops advancing when advance_on_rejection is false', function () {
        ...
        $workflow = Workflow::factory()->waterfall()->create(['advance_on_rejection' => false]);
        attachBuyers($workflow, [
            ['integration' => $buyer1, 'position' => 0],
            ['integration' => $buyer2, 'position' => 1],
        ]);
    ...
    it('handles ping-post buyer in waterfall: advances on ping rejection', function () {
        ...
        $buyer1 = Integration::factory()->pingPost()->withBuyerConfig()->create();
        $buyer2 = Integration::factory()->postOnly()->withBuyerConfig(['pricing_type' => 'fixed', 'fixed_price' => 5.0])->create();

        $workflow = Workflow::factory()->waterfall()->create(['advance_on_rejection' => true]);
        attachBuyers($workflow, [
            ['integration' => $buyer1, 'position' => 0],
            ['integration' => $buyer2, 'position' => 1],
        ]);
...
describe('Combined strategy', function () {
    ...
    it('activates secondary waterfall when primary best-bid fails', function () {
        ...
        $primary = Integration::factory()->pingPost()->withBuyerConfig()->create();
        $secondary = Integration::factory()->postOnly()->withBuyerConfig(['pricing_type' => 'fixed', 'fixed_price' => 5.0])->create();

        $workflow = Workflow::factory()->combined()->create(['cascade_on_post_rejection' => true, 'cascade_max_retries' => 1]);
        attachBuyers($workflow, [
            ['integration' => $primary,   'position' => 0, 'group' => 'primary'],
            ['integration' => $secondary, 'position' => 0, 'group' => 'secondary'],
        ]);
...
describe('Async postback pricing', function () {
    it('creates a pending_postback post result and does not mark dispatch as sold', function () {
        ...
        $buyer = Integration::factory()->postOnly()->withBuyerConfig([
            'pricing_type' => 'postback',
