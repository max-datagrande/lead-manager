...
function makePendingPostbackSetup(): array
{
    ...
    $integration = Integration::factory()->postOnly()->withBuyerConfig(['pricing_type' => 'postback'])->create();
...
it('resolves a pending postback and marks dispatch as sold', function () {
    ...
    postJson("/v1/ping-post/postback/{$dispatch->id}/{$integration->id}", ['price' => 15.50])
        ->assertOk()
...
it('accepts price from "amount" param as fallback', function () {
    ...
    postJson("/v1/ping-post/postback/{$dispatch->id}/{$integration->id}", ['amount' => 8.00])
        ->assertOk()
        ->assertJsonPath('data.status', 'postback_resolved');
...
it('returns 404 when dispatch does not exist', function () {
    ...
    postJson("/v1/ping-post/postback/999999/{$integration->id}", ['price' => 10.0])
        ->assertNotFound();
});
...
it('sets postback_received_at timestamp on resolution', function () {
    ...
    postJson("/v1/ping-post/postback/{$dispatch->id}/{$integration->id}", ['price' => 5.0])
        ->assertOk();
