...
class BuyerController extends Controller
{
    public function __construct(
        private readonly BuyerConfigService $configService,
    ) {}
    ...
    public function create(): Response|\Illuminate\Http\RedirectResponse
    {
        $integrations = Integration::whereIn('type', ['ping-post', 'post-only'])
            ->whereDoesntHave('buyer')
            ->orderBy('name')
            ->get(['id', 'name', 'type']);

        if ($integrations->isEmpty()) {
            add_flash_message('error', 'No hay integraciones ping-post o post-only disponibles. Crea una integración primero e inténtalo nuevamente.');
    ...
    public function store(StoreBuyerConfigRequest $request): \Illuminate\Http\RedirectResponse
    {
        ...
        return redirect()->route('ping-post.buyers.index')
    ...
    public function edit(Buyer $buyer): Response
    {
        ...
        return Inertia::render('ping-post/buyers/edit', [
            'buyer' => $buyer,
            'integrations' => Integration::whereIn('type', ['ping-post', 'post-only'])
                ->where(fn ($q) => $q->whereDoesntHave('buyer')->orWhere('id', $buyer->integration_id))
                ->orderBy('name')
                ->get(['id', 'name', 'type']),
            'pricingTypes' => PricingType::toArray(),
        ]);
    ...
    public function update(StoreBuyerConfigRequest $request, Buyer $buyer): \Illuminate\Http\RedirectResponse
    {
        ...
        return redirect()->route('ping-post.buyers.show', $buyer)
    ...
    public function duplicate(Buyer $buyer): \Illuminate\Http\RedirectResponse
    {
        ...
        return redirect()->route('ping-post.buyers.edit', $clone)
            ->with('success', 'Buyer duplicated — assign an integration before activating.');
    }
