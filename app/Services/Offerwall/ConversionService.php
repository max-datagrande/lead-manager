...
class ConversionService
{
  ...
  public function createConversion(array $data): OfferwallConversion
  {
    ...
    try {
      ...
      Logger::saveLog('Failed to decrypt offer_token.', 'offerwall/conversion-service', 'error', [
    ...
    if (count($parts) === 3) {
      ...
      if (!$callLog) {
        Logger::saveLog('IntegrationCallLog not found for given token.', 'offerwall/conversion-service', 'error', [
          'mixLogId' => $mixLogId,
          'parsedIntegrationId' => $parsedIntegrationId,
          'file' => __FILE__,
          'line' => __LINE__,
        ]);
      ...
      if (!$integration) {
        Logger::saveLog('Integration not found for a valid call log.', 'offerwall/conversion-service', 'error', [
          'parsedIntegrationId' => $parsedIntegrationId,
      ...
      if (!empty($callLog->response_body)) {
        $offerwallEnv = $integration->environments->where('env_type', 'offerwall')->where('environment', 'production')->first();
    ...
    if (is_null($integrationId)) {
      Logger::saveLog('Integration ID is null just before creating conversion.', 'offerwall/conversion-service', 'critical', [
        'data' => $data,
    ...
    try {
      ...
      Logger::saveLog('Offerwall conversion created successfully.', 'offerwall/conversion-service', 'info', [
        'id' => $conversion->id,
        'file' => __FILE__,
        'line' => __LINE__,
      ]);
      ...
      Logger::saveLog('Failed to create offerwall conversion in database.', 'offerwall/conversion-service', 'error', [
        'error' => $e->getMessage(),
  ...
  private function sendSlackAlert(string $title, string $message, array $contextData, array $extraFields = []): void
  {
    ...
    $slack->addTitle($title, '🚨')->addSection($message)->addDivider();
