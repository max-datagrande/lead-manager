    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          onClick={handleCopy}
          className="shrink-0 rounded p-0.5 text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
          aria-label="Copy URL"
        >
...
export function EnvironmentDetails({ integrationId, env, mappingConfig }: Props) {
  ...
  const effectiveSection: Section = navItems.some((item) => item.id === sectionParam) ? (sectionParam as Section) : 'endpoint';
    ...
    <Sheet>
      <div className="flex items-start gap-5">
        ...
        <nav className="flex w-36 shrink-0 flex-col gap-0.5 pt-0.5">
          {navItems.map(({ id, label, Icon }) => (
            <button
              ...
              className={cn(
                'flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm whitespace-nowrap transition-colors',
                effectiveSection === id ? 'bg-muted font-medium text-foreground' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground',
              )}
            >
        ...
        <div className="min-w-0 flex-1">
          ...
          {effectiveSection === 'headers' && parsedHeaders && (
            <Card className="gap-0">
              <div className="px-5 py-4">
                <p className="mb-3 text-[10px] font-medium tracking-wider text-muted-foreground uppercase">Headers</p>
                <DescriptionList>
          ...
          {effectiveSection === 'tokens' && hasMapping && (
            <Card className="gap-0">
              <div className="px-5 py-4">
                <p className="mb-3 text-[10px] font-medium tracking-wider text-muted-foreground uppercase">Token Mapping</p>
          ...
          {effectiveSection === 'response' && responseFields && (
            <Card className="gap-0">
              <div className="px-5 py-4">
                <p className="mb-3 text-[10px] font-medium tracking-wider text-muted-foreground uppercase">Response Config</p>
      ...
      <SheetContent className="w-[400px] gap-0 sm:w-[540px]">
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-auto">
          ...
          {testResult && <JsonViewer data={JSON.stringify(testResult, null, 2)} className="flex-1" title={<span>Response Data</span>} />}
        </div>
        <SheetFooter>
          <SheetClose asChild>
