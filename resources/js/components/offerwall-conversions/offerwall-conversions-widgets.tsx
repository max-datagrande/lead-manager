...
export function OfferwallConversionsWidgets({ totalPayout, isLoading = false, totalConversions }: OfferwallConversionsWidgetsProps) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? <Skeleton className="h-8 w-[100px]" /> : <div className="text-2xl font-bold">{totalConversions}</div>}
          <p className="text-xs text-muted-foreground">Total conversions from current search</p>
        </CardContent>
      </Card>
    </div>
  );
