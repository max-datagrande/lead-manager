...
> = {
  ...
  completed: { label: 'Completed', icon: CheckCircle, className: 'border-green-200 bg-green-100 text-green-800 dark:bg-green-950 dark:text-green-300' },
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start">
        <DropdownMenuItem
          ...
          onClick={() => modal.open(<DispatchLogsViewer execution={execution} />, { maxWidth: 'sm:max-w-2xl' })}
        >
          View Dispatch Logs
        </DropdownMenuItem>
      </DropdownMenuContent>
...
export const createExecutionColumns = (): ColumnDef<PostbackExecution>[] => [
      ...
      return (
        <div className="flex items-center gap-2 w-[130px]">
          ...
          <CopyToClipboard textToCopy={uuid}>
            <span className="cursor-help font-mono text-xs" title={uuid}>
              {uuid.slice(0, 8)}…
            </span>
          </CopyToClipboard>
        </div>
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col gap-0.5">
          <span className="text-sm font-medium">{execution.postback?.name ?? '—'}</span>
          {execution.postback && (
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="cursor-help font-mono text-xs">
              {preview}{hasMore ? ` +${entries.length - 2} more` : ''}
            </span>
          </TooltipTrigger>
          <TooltipContent className="bg-black text-white max-w-xs" arrowClassName="bg-black fill-black">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="cursor-help font-mono text-xs max-w-[180px] truncate block">{url}</span>
          </TooltipTrigger>
          <TooltipContent className="bg-black text-white max-w-sm" arrowClassName="bg-black fill-black">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className="text-xs whitespace-nowrap">{val ? formatDateTime(val) : '—'}</span>
    },
    enableSorting: true,
    enableHiding: true,
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-xs whitespace-nowrap">{formatDateTime(row.getValue('created_at'))}</span>
    ),
    enableSorting: true,
    enableHiding: true,
  },
