...
export function FieldSearchPopover({ fields = [], position, onSelect, onClose }) {
  ...
  const filtered = fields.filter(
    (f) => f.name.toLowerCase().includes(search.toLowerCase()) || (f.label ?? '').toLowerCase().includes(search.toLowerCase()),
  );
  ...
  return (
    ...
    >
      ...
      <div className="max-h-56 overflow-y-auto py-1">
        {filtered.length === 0 && <p className="px-3 py-4 text-center text-xs text-muted-foreground">No fields found.</p>}
        {filtered.map((field, i) => (
          ...
          >
            <span className="text-xs leading-none font-medium">{field.label ?? field.name}</span>
