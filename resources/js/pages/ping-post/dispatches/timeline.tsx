...
import type { DispatchAttemptSummary, DispatchTimelineLog, LeadDispatch } from '@/types/ping-post';
...
const TimelinePage = ({ dispatch, timelineLogs, allAttempts = [] }: Props) => {
  ...
  return (
    <>
      ...
      <div className="relative flex-1 space-y-6 p-6 md:p-8">
        <PageHeader title="Timeline Log" description={`${workflowName} · ${strategyLabel}`} smallText={dispatch.dispatch_uuid}>
        ...
        <AttemptTabs attempts={allAttempts} currentDispatchId={dispatch.id} buildHref={(id) => route('ping-post.dispatches.timeline', id)} />
